...
private[scynapse] class DispatchPayloadToAkkaInterceptor[T <: Message[_]](dispatchTo: ActorRef)
    extends MessageDispatchInterceptor[T] {

  override def handle(messages: util.List[_ <: T]): BiFunction[Integer, T, T] = (i: Integer, t: T) => {
    dispatchTo ! t.getPayload
    t
  }
}

private[scynapse] class DispatchMessageToAkkaInterceptor[T <: Message[_]](dispatchTo: ActorRef)
    extends MessageDispatchInterceptor[T] {
...
private[scynapse] class SubscriptionManager(eventBus: AxonEventBus) extends Actor with ActorLogging {
